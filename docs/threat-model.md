...
| **Date** | 2026-09-16 |
| **Status** | **Accepted by the maintainer, 2026-09-16.** Three minor questions still open in §4.14 (Q15, Q17, Q19). None of them change a triage outcome. |
...
| --- | --- | --- | --- |
| **A. Definition loading** | `init`, `loadDocument`, `validateDefinition`, `refparser.ts`, `dereference-json-schema` (quick mode) | **Yes.** Reads local files and fetches HTTP(S) URLs to resolve external `$ref`s via `@apidevtools/json-schema-ref-parser` (default `resolve.external: true`) *(maintainer, 2026-09)* | In, as a **trusted-input** boundary (§4.4) |
...
6. `authorized === false` with requirements present: `unauthorizedHandler` if registered → **early return**. No handler and `strict: true` → **reject with `401-unauthorized`**. No handler and `strict: false` → warn once, fall through (§4.5a, §4.9).
...
| B, C, D | it's reachable from the five `Request` fields with any well-formed definition and default options, **or** with a non-default option that isn't dev-only per §4.5a. |
...
- **Filesystem / network:** family A only (§4.2), only for the definition path and the external `$ref`s it names *(maintainer, 2026-09)*.
...
**So what's the insecure default story?** In 5.x: not registering `unauthorizedHandler` or `validationFail` is a supported non-strict configuration, you get warned once, and `strict: true` is one line away. In 6.0 the choice goes away and the library fails closed. That's the migration path, and it's the ruling on GHSA-7mmm-8m7g-cp5g (§4.13).
...
| `mockResponseForOperation` | `operationId`, `opts.code/mediaType/example` | **no, trusted by contract.** If you forward client values here, the lookups are plain property reads on the definition (`responses[opts.code]`, `examples[opts.example]`) with no prototype key guard *(inferred)* | see §4.11, Q15 |
...
- **No path canonicalisation** (§4.6). `/a/../b`, `/a//b` and `/%61` are three different paths to this router, whatever your proxy thinks *(maintainer, 2026-09)*.
...
| "Security scheme `X` has no handler and the request isn't rejected" | Missing handler = failed scheme (P1). Whether the request gets *rejected* depends on `unauthorizedHandler` / `strict` (§4.5a). In non-strict mode: `BY-DESIGN: property-disclaimed` (§4.9). |
...
| "Invalid body reaches operation handler" | Non-strict, no `validationFail` → `BY-DESIGN` (§4.9 false friend 2). Otherwise `VALID` (P4). |
| "Body under `text/plain` / `application/xml` / `multipart` isn't validated" | §4.9 "no content-type enforcement". `BY-DESIGN`. |
...
- **6.0 removing the non-strict fall-through.** Rewrites §4.5a, §4.9 bullets 2 and 3, §4.10 items 1 and 2, false friends 1, 2 and 5, and the GHSA-7mmm row below.
...
- **Evidence the model is incomplete:** any report that can't be routed to exactly one §4.13 disposition. Fix by editing §4.8 or §4.9, not by ad-hoc triage.
...
| --- | --- | --- |
| `VALID` | Breaks P1 to P8 via the client adversary and an in-model input. | §4.8, §4.6, §4.7 |
| `VALID-HARDENING` | No property broken, but a §4.11 misuse is easy enough that we choose to harden. Private report, maintainer discretion, typically no CVE. | §4.11 |
...
| `OUT-OF-MODEL: non-default-build` | Only shows up under a weakening `ajvOpts` / `customizeAjv`. | §4.5a |
| `BY-DESIGN: property-disclaimed` | About a §4.9 non-property or false friend. Includes the non-strict fall-through in 5.x. | §4.9 |
...
| GHSA-7mmm-8m7g-cp5g: fail-open without `unauthorizedHandler` | `VALID-HARDENING`. Non-strict fall-through stays by design in 5.x. Hardened in 5.21.0: once-only warning, and `strict: true` rejects. Fail-closed unconditionally in 6.0. The advisory should be updated to say so and credit EQSTLab. | §4.5a, §4.9, P2 |
...
15. **`mockResponseForOperation` / `validateRequest(…, operationIdString)` arguments are trusted.** *Proposed:* yes. The only reason to say otherwise would be to add prototype key guards on the definition lookups. → §4.6.
...
19. **Ownership and revision.** *Proposed:* the maintainer owns this file. It gets updated in the same PR as any change listed in §4.12. The confidence line gets updated as questions close. → header.
